    ...
    <div class="main-content" :style="{ marginLeft: isSidebarCollapsed ? '70px' : '200px' }">
        ...
        <div class="text-content">
          <div class="text-wrapper">
            <h1 class="add-master-text">Paket Wisata</h1>
            <p class="data-setup-text">Rincian Paket</p>
          </div>
          <router-link to="/tour-packages" custom v-slot="{ navigate }">
  ...
  try {
    ...
    this.packages = response.data.data
      .filter(pkg => pkg.category_program && 
                    pkg.category_program.toLowerCase() === "wisata")
      .map(pkg => ({
        ...pkg,
...
formatNumber(num) {
  ...
  try {
    ...
    this.programs = response.data.data.filter(program => 
      ...
      program.category_program.toLowerCase() === "wisata"
    );
...
updateHargaFields() {
  ...
  // Method untuk memformat input harga menjadi format Rupiah
  formatRupiah(input) {
    let angka = input.value.replace(/\D/g, ""); // Hapus semua karakter non-digit
    angka = new Intl.NumberFormat("id-ID").format(angka); // Format angka dengan pemisah ribuan
