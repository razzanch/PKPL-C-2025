    ...
    <div class="main-content" :style="{ marginLeft: isSidebarCollapsed ? '70px' : '200px' }">
        ...
        <div class="text-content">
          <div class="text-wrapper">
            <h1 class="add-master-text">Paket Haji</h1>
            <p class="data-setup-text">Rincian Paket</p>
          </div>
          <router-link to="/hajj-packages" custom v-slot="{ navigate }">
  ...
  try {
    ...
    this.packages = response.data.data
      .filter(pkg => pkg.category_program && 
                    pkg.category_program.toLowerCase() === "haji")
      .map(pkg => ({
        ...pkg,
...
formatNumber(num) {
  ...
  try {
    ...
    this.programs = response.data.data.filter(program => 
      ...
      program.category_program.toLowerCase() === "haji"
    );
...
updateHargaFields() {
    ...
    // Reset semua field (sembunyikan dan hapus required)
    ...
    // Jika dropdown memiliki value kosong, semua tetap tersembunyi
    if (!value) {
      return;
    }

    // Tampilkan sesuai pilihan dan tambahkan required
    if (value === "1") {
  ...
  async addPaket() {
    ...
    // Fungsi untuk format angka ke bilangan bulat dari input Dollar
const formatToDecimal = (value) => {
  if (!value || value.trim() === "") return null;
  // Hapus simbol $ dan koma, ambil hanya angka
  const numberValue = parseFloat(value.replace(/[$,]/g, ''));
  return isNaN(numberValue) ? null : numberValue; // Langsung kembalikan angka bulat
};
...
// Method untuk memformat input harga menjadi format Dollar tanpa desimal
formatDollar(input) {
  let angka = input.value.replace(/\D/g, ""); // Hapus semua karakter non-digit
  angka = new Intl.NumberFormat("en-US", {
    style: "currency",
    currency: "USD",
    minimumFractionDigits: 0, // Tidak pakai angka desimal
    maximumFractionDigits: 0
  }).format(angka);
  input.value = angka; // Set ulang nilai input
},
  ...
  validateInput(input, label) {
    ...
    if (!validCharsRegex.test(value)) {
      this.showNotification(`Input ${label} tidak valid! Hanya boleh huruf, angka, dan karakter (. , () -).`, false);
  ...
  quadInput.addEventListener("input", () => this.formatDollar(quadInput));
  tripleInput.addEventListener("input", () => this.formatDollar(tripleInput));
  doubleInput.addEventListener("input", () => this.formatDollar(doubleInput));
  ...
  clearButton.addEventListener("click", (event) => {
   ...
   // Daftar input yang perlu divalidasi
   const inputsToValidate = [
    { id: "pre-program", label: "Program" },
  ...
  inputsToValidate.forEach(({ id, label }) => {
    const input = document.getElementById(id);
    if (input) {
      input.addEventListener("blur", () => this.validateInput(input, label));
  ...
  quadInput.removeEventListener("input", () => this.formatDollar(quadInput));
  tripleInput.removeEventListener("input", () => this.formatDollar(tripleInput));
  doubleInput.removeEventListener("input", () => this.formatDollar(doubleInput));
